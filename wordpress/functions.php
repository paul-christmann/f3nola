...
*/
	...
	wp_enqueue_style( 'custom-style', get_template_directory_uri() . '/custom.css', array(), 11);
	/*
...
require_once( get_template_directory() . '/f3-post-functions.php' );

add_action( 'load-post.php', 'f3nola_post_meta_boxes_setup' );
add_action( 'load-post-new.php', 'f3nola_post_meta_boxes_setup' );
add_action( 'save_post', 'save_f3nola_meta_boxes' );

/* Save the meta box's post metadata. */
function save_f3nola_meta_boxes($post_id) {
  /* Verify the nonce before proceeding. */
  if ( !isset( $_POST['f3nola_post_class_nonce'] ) ||
       !wp_verify_nonce( $_POST['f3nola_post_class_nonce'], basename( __FILE__ ) ) ) {
    return $post_id;
  }
  ...
  save_f3nola_meta_value('workout_date', $post_id);
  save_f3nola_meta_value('qic', $post_id);
  save_f3nola_meta_value('pax', $post_id);
}

function save_f3nola_meta_value($meta_box_key, $post_id) {
  $f3_mb_key="_f3nola_post_" . $meta_box_key;

  /* Get the posted data and sanitize it for use as an HTML class. */

  $new_meta_value = ( isset( $_POST[$f3_mb_key] ) ? $_POST[$f3_mb_key] : '' );

  /* Get the meta value of the custom field key. */
  $meta_value = get_post_meta( $post_id, $f3_mb_key, true );

  /* If a new meta value was added and there was no previous value, add it. */
  if ( $new_meta_value && '' == $meta_value ) {
    $result = add_post_meta( $post_id, $f3_mb_key, $new_meta_value, true );

  /* If the new meta value does not match the old value, update it. */
  } elseif ( $new_meta_value && $new_meta_value != $meta_value ) {
    $result = update_post_meta( $post_id, $f3_mb_key, $new_meta_value );

  /* If there is no new meta value but an old value exists, delete it. */
  } elseif ( '' == $new_meta_value && $meta_value ) {
    $result = delete_post_meta( $post_id, $f3_mb_key, $meta_value );
  }
}


/* Meta box setup function. */
function f3nola_post_meta_boxes_setup() {
  /* Add meta boxes on the 'add_meta_boxes' hook. */
  add_action( 'add_meta_boxes', 'f3nola_add_post_meta_boxes' );
}

function f3nola_add_post_meta_boxes() {
  add_meta_box(
    'f3nola-meta-boxes-class',      // Unique ID
    esc_html__( 'Workout Details', '' ),    // Title
    'f3nola_post_class_meta_box',   // Callback function
...
function add_f3nola_meta_box($meta_box_key, $meta_box_title, $post) {
  $f3_mb_key="_f3nola_post_" . $meta_box_key;
?>
  <p>
    <label for="<?php echo $f3_mb_key?>">
      <?php _e( $meta_box_title, '' ); ?>
    </label>
    <br />
    <input class="widefat" type="text"
           name="<?php echo $f3_mb_key?>"
           id="<?php echo $f3_mb_key?>"
           value="<?php echo esc_attr( get_post_meta($post->ID, $f3_mb_key, true ) ); ?>" size="30" />
  </p>
<?php
}

/* Display the post meta box. */
function f3nola_post_class_meta_box( $post ) {
  wp_nonce_field( basename( __FILE__ ), 'f3nola_post_class_nonce' );
  add_f3nola_meta_box('workout_date', 'Workout Date (MM/DD/YY)', $post);
  add_f3nola_meta_box('qic', 'QIC', $post);
  add_f3nola_meta_box('pax', 'The PAX', $post);
}
?>
